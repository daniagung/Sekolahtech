...
if ( isset( $contact_form_id ) && $contact_form_id > 0 ) { ?>
    <div class="<?php echo implode( ' ', $wrap_class ); ?>">
        ...
        if ( 'dark' === $color_form ) {
            $form_html = str_replace( 'wpcf7-form-control', 'wpcf7-form-control input-dark', $form_html );
        } else {
            $form_html = str_replace( 'wpcf7-form-control', 'wpcf7-form-control input-standard-grey', $form_html );
        }
        ...
        $button_html = '<button class="btn--with-shadow btn btn-medium btn--' . esc_attr( $color_btn . ' ' . $button_class ) . '"><span class="text">' . esc_html( $button_text ) . '</span></button>';
...
<?php } else { ?>
    <?php esc_html_e( 'Please create new and select contact form 7.', 'utouch' ); ?>
<?php } ?>
