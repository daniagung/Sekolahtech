...
<?php foreach($options['event_children'] as $key => $row) : ?>
	...
	<div class="details-event-button">
		<button data-uri="<?php echo add_query_arg( array( 'row_id' => $key, 'calendar' => 'google' ), fw_current_url() ); ?>" type="button"><?php _e('Google Calendar', 'utouch') ?></button>
		<button data-uri="<?php echo add_query_arg( array( 'row_id' => $key, 'calendar' => 'ical'   ), fw_current_url() ); ?>" type="button"><?php _e('Ical Export', 'utouch') ?></button>
	</div>
	<ul class="details-event">
		<li><b><?php _e('Start', 'utouch') ?>:</b> <?php echo esc_html($row['event_date_range']['from']) ?></li>
		<li><b><?php _e('End', 'utouch') ?>:</b> <?php echo esc_html($row['event_date_range']['to']) ?></li>

		<?php if (empty($row['event-user']) === false) : ?>
			<li>
				<b><?php _e('Speakers', 'utouch') ?>:</b>
