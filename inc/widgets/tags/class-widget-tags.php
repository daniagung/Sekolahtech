...
if ( defined( 'FW' ) ) {
	 class Utouch_Widget_Tags extends WP_Widget {
		...
		public function __construct() {
			$widget_ops = array( 'description' => esc_html__( 'Theme styled tags list', 'utouch' ), 'classname' => 'w-tags', 'customize_selective_refresh' => true );
			parent::__construct( false, esc_html__( 'Theme widget: Tags Cloud', 'utouch' ), $widget_ops );
		}
		...
		function widget( $args, $instance ) {
			if ( defined( 'FW' ) ) {
				...
				$font_size = ! empty( $instance['font_size'] ) ? $instance['font_size'] : 13;
		...
		function form( $instance ) {
			$title     = empty( $instance['title'] ) ? esc_html__( 'Tags list', 'utouch' ) : $instance['title'];
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e( 'Number tags:', 'utouch' ); ?></label>
				<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'number' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'number' )); ?>" type="text" value="<?php echo esc_attr( $number ); ?>"/>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'font_size' ) ); ?>"><?php esc_html_e( 'Font size (in px):', 'utouch' ); ?></label>
			...
			if ( ! empty( $taxonomies ) ) { ?>
				<p>
					<label for="<?php echo esc_attr($this->get_field_id( 'cat_sel' )); ?>"><?php esc_html_e( 'Select Taxonomy', 'utouch' ); ?></label>
