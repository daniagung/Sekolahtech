...
function process_simple_like() {
	...
	if ( ! wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'utouch' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = null ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'utouch' );
		$icon  = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'utouch' );
		$icon  = $icon_empty;
	}
...
function show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'utouch' ); ?></label></th>
			<td>
				<?php
				...
				<?php else : ?>
					<p><?php esc_html_e( 'You do not like anything yet.', 'utouch' ); ?></p>
					<?php
				endif;
				wp_reset_postdata();
				?>
