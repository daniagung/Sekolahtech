...
if ( ! function_exists( 'utouch_posted_time' ) ) :
	function utouch_posted_time( $icon = true ) {
...
if ( ! function_exists( 'utouch_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function utouch_posted_on() {
		...
		if ( 'yes' === $show_meta ) {
			...
			echo utouch_posted_time();

			if ( 'post' === get_post_type() ) {
				/* translators: used between list items, there is a space after the comma */
				$categories_list = get_the_category_list( esc_html__( ', ', 'utouch' ) );
				if ( $categories_list && utouch_categorized_blog() ) {
...
if ( ! function_exists( 'utouch_post_author' ) ) :
	/**
	 * Generate html markup for post author display.
	 *
	 * @param $author_id int Id of author
	 */
	function utouch_post_author( $author_id ) {
		?>
        <span class="post__author author vcard">
								<?php esc_html_e( 'By', 'utouch' ); ?>
            <a href="<?php echo get_author_posts_url( $author_id ); ?>"
               class="fn"><?php the_author_meta( 'display_name', $author_id ); ?></a>
		</span>
		<?php
	}
endif;
if ( ! function_exists( 'utouch_comments_count' ) ) :
	/**
	 * Generate html markup for post comments count display.
	 *
	 * @param $author_id int Id of author
	 */
	function utouch_comments_count() {

		if ( is_singular() ) {
			$link = '#comments';
		} else {
			$link = get_the_permalink() . '#comments';
		}
		?>
        <span class="post__comments">
			<a href="<?php echo esc_url( $link ) ?>"><?php echo esc_html( get_comments_number() ) ?>
                <span><?php echo esc_html__( 'Comments', 'utouch' ) ?></span>
			</a>
		</span>
		<?php
	}
endif;
if ( ! function_exists( 'utouch_post_author_avatar' ) ) :
	/**
	 * Generate html markup for post author display.
	 *
	 * @param $author_id int Id of author
	 */
	function utouch_post_author_avatar( $author_id ) {
		...
		if ( ! is_author() && 'yes' === $show_author ) {
            ...
            <div class="post__author author vcard">
                ...
                <div class="post__author-name fn">
					<?php esc_html_e( 'Posted by', 'utouch' ); ?>
                    <a href="<?php echo get_author_posts_url( $author_id ); ?>"
                       title="<?php echo esc_attr( get_the_author() ); ?>"
                       class="post__author-link"><?php the_author_meta( 'display_name', $author_id ); ?></a>
                </div>
            </div>
		<?php }
	}
endif;
if ( ! function_exists( 'utouch_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function utouch_entry_footer() {
...
if ( ! function_exists( 'utouch_logo' ) ):
	/**
	 * Returns logotype markup depends on theme options.
	 *
	 */
	function utouch_logo() {
		$logo_image_style = '';
		...
		echo '<a href="' . esc_url( home_url( '' ) ) . '" class="full-block" rel="home"></a>';
		if ( ! empty( $logo_image ) ) {
...
if ( ! function_exists( 'utouch_additional_nav' ) ):
	function utouch_additional_nav() {
		$show_search  = 'yes';
		...
		if ( class_exists( 'WooCommerce' ) || class_exists( 'Easy_Digital_Downloads', false ) || 'yes' === $show_search ) { ?>
            <ul class="nav-add">
				<?php if ( class_exists( 'WooCommerce' ) ) { ?>
                    <li class="cart woo-menu-cart">
						<?php get_template_part( 'parts/shop', 'cart' ); ?>
                    </li>
				<?php } ?>
				<?php if ( class_exists( 'Easy_Digital_Downloads', false ) ) { ?>
                    <li class="cart edd-menu-cart">
                        <a href="<?php echo edd_get_checkout_uri(); ?>">
                            <i class="seoicon-basket"></i> <span class="cart-count edd-cart-quantity"><?php echo edd_get_cart_quantity(); ?></span>
                        </a>
                    </li>
				<?php } ?>
				<?php if ( 'yes' === $show_search ) {
					$icon_class = 'fullscreen' === $search_style ? 'js-open-search-popup' : 'js-open-search-standard';
					echo '<li class="search search_main"><a href="#" class="' . esc_attr( $icon_class ) . '"><svg class="utouch-icon utouch-icon-search  ' . ( 'fullscreen' === $search_style ? 'cd-nav-trigger' : '' ) . '" viewBox="0 0 512 512">
      <path d="m411 203c-12-2-23 5-25 17-3 19-12 35-26 50-14 13-31 22-50 26-10 2-18 13-16 24 2 11 12 19 24 18 27-6 52-19 71-38 20-20 33-45 38-72 3-12-4-22-16-25z m-109-203c-116 0-210 94-210 209 0 32 7 61 19 88l-78 68c-56 58-25 109-9 124 14 15 66 48 124-10l69-78c26 11 54 18 85 18 115 0 209-94 209-210 0-115-94-209-209-209z m-185 452c-10 11-38 29-63 8-7-6-21-36 8-63l72-63c13 18 28 33 45 45z m185-77c-92 0-166-74-166-166 0-91 74-165 166-165 91 0 165 74 165 165 0 92-74 166-165 166z"></path>
			...
			<?php if ( 'yes' === $show_search ) {
				get_template_part( 'parts/search', $search_style );
			}
		}
	}
endif;
 ...
 * @return bool
 */
function utouch_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'utouch_categories' ) ) ) {
		...
		set_transient( 'utouch_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so utouch_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so utouch_categorized_blog should return false.
		return false;
	}
}
 ...
 */
function utouch_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'utouch_categories' );
}

add_action( 'edit_category', 'utouch_category_transient_flusher' );
add_action( 'save_post', 'utouch_category_transient_flusher' );
 ...
 * @return string
 */
function utouch_previous_comments_link_attributes( $attributes ) {
 ...
 * @return string
 */
function utouch_next_comments_link_attributes( $attributes ) {
	return $attributes . ' class="btn-next-wrap"';
}

/* Add classes to the comments pagination. */
add_filter( 'previous_comments_link_attributes', 'utouch_previous_comments_link_attributes' );
add_filter( 'next_comments_link_attributes', 'utouch_next_comments_link_attributes' );

if ( ! function_exists( 'utouch_comments' ) ) :
	/**
	 * utouch List Comments Callback
	 * callback function for wp_list_comments in utouch/comments.php
	 *
	 * @param object $comment Comment object.
	 * @param array $args Arguments for callback.
	 * @param int $depth Max. depth of comments tree.
	 */
	function utouch_comments( $comment, $args, $depth ) {
		do_action( 'utouch_comments', $comment, $args, $depth );
		...
		switch ( $comment->comment_type ) :
			...
			case 'trackback' :
                ...
                <div class="comment-content post-content comment">
                    <h5><?php esc_html_e( 'Pingback:', 'utouch' ); ?><?php comment_author_link(); ?> </h5>
					<?php edit_comment_link( esc_html__( 'Edit', 'utouch' ), '<div class="simple-article small"><span>', '</span></div>' ); ?>
                </div>
			...
			default :
                ...
                <h5 class="comment-awaiting-moderation"> <?php esc_html_e( 'Your comment is awaiting moderation.', 'utouch' ); ?></h5>
			<?php endif; ?>
                        ...
                        <div class="d-flex--content-inline">
                            ...
                            <header class="comment-meta comments__header">
                                ...
                                <cite class="fn url comments__author">
                                    <a class="comment-author-link h6" href="<?php comment_author_url(); ?>"
                                ...
                                <div class="comments__time">
                                    <time class="comment-meta-item"
                                          ...
                                          itemprop="datePublished"><?php comment_date( 'jS F Y' ) ?>
							...
							comment_reply_link( array_merge( $args, array(
								'reply_text' => esc_html__( 'Reply', 'utouch' ),
								'depth'      => $depth,
								'max_depth'  => $args['max_depth']
							) ), $comment, get_post() );
							edit_comment_link( esc_html__( 'Edit', 'utouch' ), '', '' );
							?>
                        </div>
