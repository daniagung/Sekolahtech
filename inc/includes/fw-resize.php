...
if ( ! function_exists( 'utouch_resize' ) ) {
	function utouch_resize( $url, $width = false, $height = false, $crop = false ) {
		$utouch_resize = Utouch_Resize::getInstance();
		$response  = $utouch_resize->process( $url, $width, $height, $crop );

		return ( ! is_wp_error( $response ) && ! empty( $response['src'] ) ) ? $response['src'] : $url;
	}
}
if ( ! function_exists( 'utouch_delete_resized_thumbnails' ) ) {
	function utouch_delete_resized_thumbnails( $id ) {
	...
	add_action( 'delete_attachment', 'utouch_delete_resized_thumbnails' );
}